...
import com.wlqq.bigdata.storm.hdfs.bolt.rotation.TimedRotationPolicy;
...
public abstract class AbstractHdfsBolt1 extends BaseRichBolt {
    /**
	 * 
	 */
	private static final long serialVersionUID = -8655073498092760804L;

	private static final Logger LOG = LoggerFactory.getLogger(AbstractHdfsBolt1.class);
    ...
    public void cleanup() {
        ...
        synchronized (this.writeLock) {
            ...
            LOG.error("Performing {} file rotation actions.", this.rotationActions.size());
            for (RotationAction action : this.rotationActions) {
            	try {
					action.execute(this.fs, this.currentFile);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					//e.printStackTrace();
					LOG.error("load data to hive fail",e);
				}
            }
        }
        long time = System.currentTimeMillis() - start;
